        ...
        for leg in LEG_P: 
            for sea in SEA: 
                for varset in VARSET:
                    for meth in METHODS:
                        for index_trials in range(0,INDEX_TRIALS+1,1): 
                            
                            if sep_method != 'index':
                                TST_SUBSETS = 0
                            elif sep_method == 'index': 
                                curr_inds = TRNTST_SPLIT[:,index_trials]
                                TST_SUBSETS = np.unique(curr_inds)
                                print(len(curr_inds),len(data))
                                # if len(TRNTST_SPLIT.shape) == 1:
                                #     curr_inds = TRNTST_SPLIT
                                #     TST_SUBSETS = curr_inds.unique()
                                # elif len(TRNTST_SPLIT.shape) == 2:
                                #    curr_inds = TRNTST_SPLIT.iloc[:,index_trials]
                                #    TST_SUBSETS = curr_inds.iloc[:,index_trials].unique()

            
                                # 1 are training points, 2, 3, ... are subsets of the test set on which we sequentially evaluate
                                
                            nre = 0
                            while nre < NUM_REP:

                                string_exp = sea + '_leg_' + str(leg) + '_' + sep_method + '_' + meth + '_' +  varset + '_trial_' + str(index_trials)
                                ...
                                for it_num, tst_subset in enumerate(np.setdiff1d(TST_SUBSETS,1)): # 1 is reserved for training
                                    ...
                                    if sep_method != 'index':
                                        ...
                                        # print(trn.shape[0] / (trn.shape[0] + tst.shape[0]))

                                    
                                    elif sep_method == 'index': 
                                        
                                        string_exp += '_id_tst_split_' + str(tst_subset)
                                        
                                        trn = sea_subset_.iloc[curr_inds == 1,:].copy()
                                        tst = sea_subset_.iloc[curr_inds == tst_subset,:].copy()
